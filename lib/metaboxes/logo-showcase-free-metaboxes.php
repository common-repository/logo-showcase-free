...
function pic_logo_showcase_notice_message() {
    // Show only to Admins
    ...
    $dismiss_notice  = get_option( 'pic_logo_showcase_notice_dismiss', 'no' );
    $activation_time = get_option( 'pic_logo_showcase_free_activation_time' ); // Retrieving activation time
    $days_installed = floor((time() - $activation_time) / (60 * 60 * 24)); // Calculating days since installation

    $plugin_url      = 'https://wordpress.org/support/plugin/logo-showcase-free/reviews/#new-post';

    // Nonce field
    $nonce_field = wp_nonce_field( 'pic_logo_showcase_dismiss_notice_nonce', '_nonce', true, false );
    ...
    <div id="rsbbox-review-notice" class="rsbbox-review-notice">
        <div class="testimonial-review-text">
            <h3><?php echo wp_kses_post( 'Review', 'logo-showcase-free' ); ?></h3>
            <p><?php echo wp_kses_post( 'Awesome, you\'ve been using <strong>Logo Showcase Plugin</strong> for more than 1 week. May we ask you to give it a <strong>5-star rating</strong> on Wordpress? </br>
                    ...
                    <br><br>Your help is much appreciated. Thank you very much', 'logo-showcase-free' ); ?></p>
            <ul class="testimonial-review-ul">
                <li><a href="<?php echo esc_url( $plugin_url ); ?>" target="_blank"><span class="dashicons dashicons-external"></span><?php esc_html_e( 'Sure! I\'d love to!', 'logo-showcase-free' ); ?></a></li>
                <li><a href="#" class="notice-dismiss" data-nonce="<?php echo esc_attr(wp_create_nonce('pic_logo_showcase_dismiss_notice_nonce')); ?>"><span class="dashicons dashicons-smiley"></span><?php esc_html_e( 'I\'ve already left a review', 'logo-showcase-free' ); ?></a></li>
                <li><a href="#" class="notice-dismiss" data-nonce="<?php echo esc_attr(wp_create_nonce('pic_logo_showcase_dismiss_notice_nonce')); ?>"><span class="dashicons dashicons-dismiss"></span><?php esc_html_e( 'Never show again', 'logo-showcase-free' ); ?></a></li>
            </ul>
        </div>
    </div>

    <style type="text/css">
        #rsbbox-review-notice .notice-dismiss{
            padding: 0 0 0 26px;
        }
        #rsbbox-review-notice .notice-dismiss:before{
            display: none;
        }
        #rsbbox-review-notice.rsbbox-review-notice {
            padding: 15px;
        ...
        #rsbbox-review-notice .testimonial-review-text {
            overflow: hidden;
        }
        #rsbbox-review-notice .testimonial-review-text h3 {
            font-size: 24px;
            margin: 0 0 5px;
            font-weight: 400;
            line-height: 1.3;
        }
        #rsbbox-review-notice .testimonial-review-text p {
            font-size: 15px;
            margin: 0 0 10px;
        }
        #rsbbox-review-notice .testimonial-review-ul {
            margin: 0;
            padding: 0;
        }
        #rsbbox-review-notice .testimonial-review-ul li {
            display: inline-block;
            margin-right: 15px;
        }
        #rsbbox-review-notice .testimonial-review-ul li a {
            display: inline-block;
            color: #2271b1;
        ...
        #rsbbox-review-notice .testimonial-review-ul li a span {
            position: absolute;
            left: 0;
            top: -2px;
        }
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $('.notice-dismiss').on('click', function(e) {
                ...
                var data = {
                    action: 'piclogoshowcase_dismiss_review_notice',
                    _nonce: nonce,
                    dismissed: true // Indicate that the notice is being dismissed
                };

                $.post(ajaxurl, data, function(response) {
                    $('#rsbbox-review-notice').remove();
                });
            });
        });
    </script>
...
add_action( 'admin_notices', 'pic_logo_showcase_notice_message' );

function pic_logo_showcase_dismiss_notice() {
    check_ajax_referer( 'pic_logo_showcase_dismiss_notice_nonce', '_nonce' ); // Verifying nonce

    if ( ! current_user_can( 'manage_options' ) ) {
        wp_send_json_error( __( 'Unauthorized operation', 'logo-showcase-free' ) );
    }

    if ( ! isset( $_POST['_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['_nonce'] ), 'pic_logo_showcase_dismiss_notice_nonce' ) ) {
        wp_send_json_error( __( 'Unauthorized operation', 'logo-showcase-free' ) );
    }

    if ( isset( $_POST['dismissed'] ) ) {
        update_option( 'pic_logo_showcase_notice_dismiss', 'yes' );
        wp_send_json_success( __( 'Notice dismissed successfully', 'logo-showcase-free' ) );
    } else {
        wp_send_json_error( __( 'Dismissal data missing', 'logo-showcase-free' ) );
    }
}
add_action( 'wp_ajax_piclogoshowcase_dismiss_review_notice', 'pic_logo_showcase_dismiss_notice' );